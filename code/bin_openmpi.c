...
float *dragonfly(unsigned int dimensions, unsigned int N,
                 unsigned int iterations, unsigned int nr_threads,
                 float space_size, Weights weights,
  ...
  float *positions =
      init_array_parallel(N * dimensions, space_size, nr_threads);
  float *speeds =
      init_array_parallel(N * dimensions, space_size / 20.0, nr_threads);
  ...
  for (unsigned int i = 0; i < iterations; i++) {
    ...
    parallel_sum(cumulated_pos, average_speed, positions, speeds, N, dimensions,
                 nr_threads);
...
int main() {
  ...
  float *res = dragonfly(dim, tot_dragonflies, iters, nr_threads, 2.0, w,
                         sphere_fitness);
