...
// struct used to store the progression of the dragonfly weights s, a, c, f, e,
// w, l (linear)
...
typedef struct {
  unsigned int population_size, starting_chunk_count, iterations,
      problem_dimensions, threads_per_process;
} Parameters;

Parameters parameter_parse(int argc, char *argv[]);

// void message_broadcast(Message *message, unsigned int index, int n,
